...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    dxl_comm_result2, dxl_error2 = packetHandler.write2ByteTxRx(portHandler, 7, ADDR_MX_GOAL_POSITION, int(512*(ServoAngle/150 +1)))
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            result = det.cpu().numpy()
            if len(result) > 0:
                BboxAngle = -(result[0][4]*180/math.pi + 90)
            else:
                BboxAngle = 0
            Difference = ServoAngle - BboxAngle
            LOGGER.info(f'BboxAngle : {BboxAngle}, ServoAngle : {ServoAngle}, Difference : {Difference}')
            ...
            ServoAngle = ServoAngle - Difference
