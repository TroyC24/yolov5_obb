...
class LoadStreams:
    ...
    def __init__(self, sources='streams.txt', img_size=[640, 640], stride=32, auto=True):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            _, init_img = cap.read()  # guarantee first frame
            self.w = img_size[0]
            self.h = img_size[1]
            self.w_scaled = int(self.initial_w*self.h/self.initial_h)
            img_re = cv2.resize(init_img, (self.w_scaled, self.h))
            self.offset = int((self.w_scaled - self.w)/2)
            self.imgs[i] = img_re[:, self.offset:self.w + self.offset] # 240:1680
    ...
    def update(self, i, cap, stream):
        ...
        while cap.isOpened() and n < f:
            ...
            if n % read == 0:
                ...
                success, init_im = cap.retrieve()
                img_re = cv2.resize(init_im, (self.w_scaled, self.h))
                im = img_re[:, self.offset:self.w + self.offset]
                if success:
                    self.imgs[i] = im
                else:
